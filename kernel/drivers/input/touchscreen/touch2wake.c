/*
 * drivers/input/touchscreen/touch2wake.c
 *
 *
...
static struct input_dev * touch2wake_pwrdev;
...
{
	if (strcmp(s2w, "3") == 0) {
		pr_debug("[cmdline_s2w]: Sweep2Sleep enabled. | s2w='%s'", s2w);
		sweep2wake = 3;
	} else if (strcmp(s2w, "2") == 0) {
		pr_debug("[cmdline_s2w]: Sweep2Wake/Sweep2Sleep enabled. | s2w='%s'", s2w);
		sweep2wake = 2;
	} else if (strcmp(s2w, "1") == 0) {
		pr_debug("[cmdline_s2w]: Sweep2Wake enabled. | s2w='%s'", s2w);
		sweep2wake = 1;
	} else if (strcmp(s2w, "0") == 0) {
		pr_debug("[cmdline_s2w]: Sweep2Wake disabled. | s2w='%s'", s2w);
		sweep2wake = 0;
	}else {
		pr_debug("[cmdline_s2w]: No valid input found. Sweep2Wake disabled. | s2w='%s'", s2w);
		sweep2wake = 0;
	}
	return 1;
}
...
/* PowerKey setter */
void touch2wake_setdev(struct input_dev * input_device) {
	touch2wake_pwrdev = input_device;
#if TW_DEBUG
	pr_debug("[TOUCH2WAKE]: power button cached\n");
#endif
	return;
}
EXPORT_SYMBOL(touch2wake_setdev);

static void reset_touch2wake(void)
{
#if TW_DEBUG
	pr_debug("[TOUCH2WAKE]: ressetting in s2w\n");
	pr_debug("[TOUCH2WAKE]: line : %d | func : %s\n", __LINE__, __func__);
#endif
	//reset sweep2wake
	tripoff = 0;
	tripon = 0;
...
static void sweep2wake_presspwr(struct work_struct * sweep2wake_presspwr_work) {
	if (!mutex_trylock(&pwrkeyworklock))
                return;
#if TW_DEBUG
	pr_debug("[SWEEP2WAKE]: pressing power\n");
#endif
        reset_touch2wake();
	input_event(touch2wake_pwrdev, EV_KEY, KEY_POWER, 1);
	input_event(touch2wake_pwrdev, EV_SYN, 0, 0);
	msleep(DEFAULT_S2W_PWRKEY_DUR);
	input_event(touch2wake_pwrdev, EV_KEY, KEY_POWER, 0);
	input_event(touch2wake_pwrdev, EV_SYN, 0, 0);
	msleep(DEFAULT_S2W_PWRKEY_DUR);
...
#if TW_DEBUG
	pr_debug("[SWEEP2WAKE]: power triggered\n");
#endif
...
#if TW_DEBUG
	pr_debug("[SWEEP2WAKE]: inside single touch\n");
#endif
	if (s2w_st_flag == 1)
		return true;
	else
...
#if TW_DEBUG
        pr_debug("[sweep2wake]: sweep_coord,sweep_height(%4d,%4d) single:%s\n",
...
#if TW_DEBUG
        pr_debug("[sweep2wake]:left to right x,y(%d,%d) jiffies:%lu\n", sweep_coord, sweep_height, time);
#endif
        if (sweep_coord < 100) {
        tripon = 1;
        triptime = time;
...
#if TW_DEBUG
        pr_debug("[sweep2wake]:right to left x,y(%d,%d) jiffies:%lu\n", sweep_coord, sweep_height, time);
#endif
        if (sweep_coord > 400) {
		tripoff = 1;
		triptime = time;
...
void doubletap2wake_func(int x, int y, unsigned long time)
{
	...
	int delta_y = 0;
#if TW_DEBUG
	pr_debug("[dt2wake]: x,y(%d,%d) jiffies:%lu\n", x, y, time);
#endif
	...
	if (time - initial_time > DT2W_TIMEOUT_MAX) {
		reset_touch2wake();
#if TW_DEBUG
		pr_debug("[TOUCH2WAKE]: d2w max timeout reset\n");
#endif
		return;
	}
	...
	if ((dt2w_time[0] - dt2w_time[1]) < 10) {
		reset_touch2wake();
#if TW_DEBUG
		pr_debug("[TOUCH2WAKE]: d2w min timeout reset\n");
#endif
		return;
	}
#if TW_DEBUG
	pr_debug("[TOUCH2WAKE]: checking d2w\n");
#endif
	dt2w_x[1] = dt2w_x[0];
	dt2w_x[0] = x;
	dt2w_y[1] = dt2w_y[0];
...
#if TW_DEBUG
		pr_debug("[dt2wake]: y = %d, timedelta = %lu, deltax= %d, deltay = %d\n",y,(dt2w_time[0] - initial_time), delta_x, delta_y);
#endif
		if (y > 50 && y < 1200
...
static int __init touch2wake_init(void)
{
	int sysfs_result;
	...
	pr_info("[touch2wake]: %s done\n", __func__);
	return sysfs_result;
}

static void __exit touch2wake_exit(void)
{
	return;
}

module_init(touch2wake_init);
module_exit(touch2wake_exit);

MODULE_DESCRIPTION("TouchWake");
