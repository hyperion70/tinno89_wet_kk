/*
 * drivers/input/touchscreen/touchwake.c
 *
 *
...
#if TW_DEBUG
	pr_debug("[SWEEP2WAKE]: power button cached\n");
#endif
	return;
}
...
{
#if TW_DEBUG
	pr_debug("[SWEEP2WAKE]: ressetting in s2w\n");
...
void doubletap2wake_func(int x, int y, unsigned long time)
{
	...
	if (time - initial_time > DT2W_TIMEOUT_MAX) {
		reset_sweep2wake();
#if TW_DEBUG
		pr_debug("[SWEEP2WAKE]: d2w max timeout reset\n");
#endif
		return;
	}
	...
	if ((dt2w_time[0] - dt2w_time[1]) < 10) {
		reset_sweep2wake();
#if TW_DEBUG
		pr_debug("[SWEEP2WAKE]: d2w min timeout reset\n");
#endif
		return;
	}
#if TW_DEBUG
	pr_debug("[SWEEP2WAKE]: checking d2w\n");
#endif
	dt2w_x[1] = dt2w_x[0];
	dt2w_x[0] = x;
	dt2w_y[1] = dt2w_y[0];
...
#if TW_DEBUG
		pr_debug("[SWEEP2WAKE]: y = %d, timedelta = %lu, deltax= %d, deltay = %d\n",y,(dt2w_time[0] - initial_time), delta_x, delta_y);
#endif
		if (y > 50 && y < 1200
